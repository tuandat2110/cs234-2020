...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.sp = tf.placeholder(dtype=tf.uint8, shape=[None, img_height, img_width, nchannels*config.state_history], name='next-sate')
        self.done_mask = tf.placeholder(dtype=tf.bool, shape=[None], name='done-mask')
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        state_flatten = tf.layers.flatten(state, name=scope)
        out = tf.layers.dense(state_flatten, num_actions, reuse=reuse, name=scope)
