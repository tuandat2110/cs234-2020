...
class BaselineNetwork(object):
  ...
  def add_baseline_op(self, scope = "baseline"):
    ...
    self.baseline = tf.squeeze(build_mlp(self.observation_placeholder, 1, scope, self.config.n_layers, self.config.layer_size, None), axis=-1)
  ...
  def update_baseline(self, returns, observations):
    ...
    baseline = self.sess.run(self.update_baseline_op, feed_dict={self.baseline_target_placeholder: returns, self.observation_placeholder: observations})
