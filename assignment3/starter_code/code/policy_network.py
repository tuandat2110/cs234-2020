...
class PG(object):
  ...
  def add_placeholders_op(self):
    ...
    if self.discrete:
        ...
        self.action_placeholder = tf.placeholder(tf.int32, shape=(None, self.action_dim))
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    #else:
    #    action_means = build_mlp(self.observation_placeholder, self.action_dim, scopt, self.config.n_layers, self.config.layer_size, None)

    #    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
    #        log_std = tf.get_variable('log_std', shape=(self.action_dim,))
    #    action_stds = tf.exp(log_std)
    #    self.sampled_action = (tf.random_normal(shape=(self.action_dim,)) * action_stds + action_means)
    #    mvn = tf.contrib.distributions.MultivariateNormalDiag(loc=action_means, scale_diag=action_stds)
